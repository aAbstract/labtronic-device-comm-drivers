...
def create_ltd_driver_ffi() -> CDLL | None:
    ltd_driver_lib_path = 'build/ltd_driver.so'
    if not os.path.exists(ltd_driver_lib_path):
        return None
    ltd_driver = CDLL(ltd_driver_lib_path)

    ltd_driver.init_ltd_driver.argtypes = [
        c_uint16,  # protocol_version
        POINTER(MsgTypeConfig),  # driver_config
        c_uint8,  # arr_size
    ]
    ltd_driver.init_ltd_driver.restype = c_uint8

    ltd_driver.encode_packet.argtypes = [
        c_uint16,  # msg_seq_number
        c_uint8,  # msg_type
        POINTER(c_uint8),  # msg_value_ptr
        POINTER(c_uint8),  # out_packet
    ]
    ltd_driver.encode_packet.restype = c_uint8

    ltd_driver.decode_packet.argtypes = [
        POINTER(c_uint8),  # packet
        POINTER(DeviceMsg),  # out_device_msg
    ]
    ltd_driver.encode_packet.restype = c_uint8

    ltd_driver.get_msg_type_config.argtypes = [
        c_uint8,  # config_idx
    ]
    ltd_driver.get_msg_type_config.restype = MsgTypeConfig
